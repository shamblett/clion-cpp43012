...
Estimate<double> bootstrap(double confidence_level, double* first, double* last, sample const& resample,
...
Config::Config(ConfigData const& data) : m_data(data)
{
    ...
    for (auto const& reporterSpec : m_data.reporterSpecifications)
    {
        ...
        m_processedReporterSpecs.push_back(ProcessedReporterSpec{
            reporterSpec.name(), reporterSpec.outputFile() ? *reporterSpec.outputFile() : data.defaultOutputFilename,
...
void Config::readBazelEnvVars()
{
    ...
    if (bazelRandomSeed)
    {
        ...
        if (!parsedSeed)
        {
            ...
            Catch::cerr() << "Warning: could not parse 'TEST_RANDOM_SEED' ('" << bazelRandomSeed
                          << "') as proper seed.\n";
        }
        else
        {
...
ScopedMessage::ScopedMessage(ScopedMessage&& old) noexcept : m_messageId(old.m_messageId)
{
    old.m_moved = true;
}
...
void setUpGuardFile(std::string const& guardFilePath)
{
    if (!guardFilePath.empty())
    {
        ...
        if (!file)
        {
            ...
            CATCH_RUNTIME_ERROR("Could not open the exit guard file '" << guardFilePath << "' because '" << errMsg
                                                                       << "' (" << err << ')');
        }
...
SectionStats::SectionStats(SectionInfo&& _sectionInfo, Counts const& _assertions, double _durationInSeconds,
...
TestCaseStats::TestCaseStats(TestCaseInfo const& _testInfo, Totals const& _totals, std::string&& _stdOut,
...
Clara::Parser makeCommandLineParser(ConfigData& config)
{
    ...
    auto cli =
        ...
        | Opt(setShardIndex,
              "shard index")["--shard-index"]("index of the group of tests to execute (see --shard-count)")
        | Opt(config.allowZeroTests)["--allow-running-no-tests"]("Treat 'No tests run' as a success")
        | Opt(config.prematureExitGuardFilePath, "path")["--premature-exit-guard-file"](
...
JsonArrayWriter::JsonArrayWriter(std::ostream& os, std::uint64_t indent_level) : m_os{os}, m_indent_level{indent_level}
{
    m_os << '[';
}
    ...
    void close() override
    {
        ...
        const bool should_wait_for_child = [&]()
        {
            ...
            if (std::find_if(m_children.begin(), m_children.end(),
                             [](TestCaseTracking::ITrackerPtr const& tracker) { return tracker->hasStarted(); })
                != m_children.end())
            {
                return false;
            }
            ...
            {
                if (child->isSectionTracker() && static_cast<SectionTracker const&>(*child).trimmedName() == filters[0])
                {
                    return true;
                }
            }
...
void RunContext::assertionEnded(AssertionResult&& result)
{
    ...
    else if (!result.succeeded())
    {
        ...
        if (result.isOk())
        {
                     ...
                     .okToFail()) // Read from a shared state established before the threads could start, this is fine
                 ...
                 NameAndTags const& nameAndTags) noexcept
{
    CATCH_TRY
    {
        getMutableRegistryHub().registerTest(makeTestCaseInfo(extractClassName(classOrMethod), nameAndTags, lineInfo),
                                             CATCH_MOVE(invoker));
    }
    CATCH_CATCH_ALL
    {
